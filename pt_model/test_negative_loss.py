...
class NEG_loss(nn.Module):
    ...
    def forward(self, input_labels, out_labels, num_sampled):
        ...
        # input = self.in_embed(Variable(input_labels.repeat(1, window_size).contiguous().view(-1)))
        output = self.out_embed(Variable(out_labels.view(-1)))
